...
try:
    ...
    dsl_ctrl.waits(10)  # Give the scenario some time to change...
    # dsl_ctrl.modify_entries(NUM_ENTRIES, 'all', parallel=IN_PARALLEL)  # Add entries to each server (in parallel)
    # dsl_ctrl.waits(10)  # Give the scenario some time to change...
    # dsl_ctrl.delete_entries(NUM_ENTRIES, 'all', parallel=IN_PARALLEL)  # Add entries to each server (in parallel)
    # dsl_ctrl.waits(10)  # Give the scenario some time to change...
    dsl_ctrl.wait_until_all_partitions_same_state(PARTITIONS, NUM_ENTRIES*2)  # wait until all servers have the same number of entries this will loop if there is no consistency!
